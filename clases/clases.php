...
class usuarios{
	...
	public static function editar($CodUsua, $datos)
	{
        ...
        /*El valor $CodUsua (siendo un valor int en la base de datos registrada), se recibe como parámetro en esta función , los índices de los arreglos solo son para correlacionar los elementos del arreglo con los valores de los arrays asociativos sobre la consulta SQL, los $datos deben ser un arreglo en el cual se introduce los valores de nombre, usuario ,profesión, país, foto_perfil que serían los datos a editar y si no simplemente se han dejado por defecto los campos en los cuales no se ha editado, no necesitamos de -> $resultado = $consulta->fetchAll();
        porque no retornaremos resultados, pues con header('location: editar_perfil.php'); ya se actualizarían en la pantalla los datos que previamente fueron actualizados en la base de datos */
...
class post{
	...
	public static function post_por_usuario($CodUsua)
	{
        ...
        /*P.CodUsua = :CodUsua es necesario, porque si bien el código SQL inmediato anterior permite hacer una relación con CodUsua entre las tablas "usuarios" con "post", esto es genérico y para todos los registros, para aplicar espacifidad  necesitamos filtrar por el $CodUsua enviado como parámetro en esta función con respecto a P.CodUsua de Post, recordando la necesidad del INNER JOIN es debido a que necesitamos más datos de los registros del usuario para llamar a datos como nombre, foto_perfil del usuario y justamente las columnas que se ha seleccionado en SELECT , el ORDER BY P.CodPost DESC implicará un orden descendente de mayor a menor, si bien no tenemos registrado una fecha, el CodPost esta programado como AUTO-INCREMENT, TODO:por ello los POST más recientes serán los valores más altos de CodPost */
		$consulta->execute(array(':CodUsua' => $CodUsua));
		$resultado = $consulta->fetchAll();
		return $resultado;
	}
	...
	{
		/* Función para mostrar todas las publicaciones de los amigos del usuario de la sesión, porque en amigos::codigos_amigos($_SESSION['CodUsua']); esta función se ejecuta en index.html inmediatamente "antes" de esta función mostrarTodo($amigos)  , se envía como parámetro $_SESSION['CodUsua']), luego desde index.php se llama con $post = post::mostrarTodo($amigos[0]['amigos'] al ejecutar esta última se puede capturar en publicacion.php el valor retornado con :
		<?php if(!empty($post)): ?>
        ...
        La importancia de usar la variable $amigo y no un array asociativo, es porque al usar la variable $amigos y no :amigos es  aseguramos que se reconocerán valores enteros devido a los pares usua_enviador, usua_receptor que los tenemos devido a $amigos = amigos::codigos_amigos($_SESSION['CodUsua']);*/
	}
...
class comentarios{
	...
	public static function mostrar($CodPost)
	{
		...
		$resultado = $consulta->fetchAll();
		return $resultado;
		/* TODO: Se ha arreglado ,es importante el ORDER BY, para justamente ir ordenando las publicaciones por el código de comentario que es unívoco y es autoincrement, como en publicaciones.php , el comentario involucra a un codPost específico y cada uno tiene varios comentarios (los procesados en esta función), solo basta ordenar los comentarios en base a CodCom en el cual cada registro de la consulta SQL  tienen asociado un nombre y el contenido de ese comentario  */
...
class mg
{
    ...
    */
	...
	{
		/*Para verificar si un usuario ya ha dado ok a una publicacion, retornará el conteo (count) de los resultados, no nos interesa cuantos me gusta, solo si hay por lo menos un me gusta. $CodUsua como parametro contiene $_SESSION['CodUsua'] <?php if(mg::verificar_mg($posts['CodPost'], $_SESSION['CodUsua']) == 0): ?>
		<a href="<?php echo $_SERVER['PHP_SELF'] ?>?mg=1&&CodPost=<?php echo $posts['CodPost'] ?>" class="like icon-checkmark2"></a>
		<?php /* mg=1 implica que hay cero me gusta por parte del usuario puede existir muchos me gustas, pero nos estamos enfocando en el usaurio de sesión, en este caso, el ícono tendrá un contorno de visto vacío si no ha puesto me gusta( lo mas común), y si ha puesto me gusta se mostrará un ícono con un contorno completo con la siguiente sección de codigo de publicacion.php
		<?php else: ?>
...
class notificaciones
{
	...
	public static function mostrar($CodUsua)
	{
		...
		$resultados = $consulta->fetchAll();
		return $resultados;
        /*Debe existir un INNER JOIN entre POST usando el cod de las notificaciones y la accion 1 o 0 que representan si ha visto o no ha visto el usuario una notificación y el CodPost que representa al codigo unívoco de cada POST que es una llave foránea para POST de un usuario , necesitamos de la tabla "Usuarios" porque desplegaremos el usuario en cada notificacion*/

	}

	public static function vistas($CodNot)
	{
		/* Se llama a esta función desde header.php. Para actualizar el campo visto de la tabla notificaciones */

		$con = conexion();
		$consulta = $con->prepare("update notificaciones set visto = 1 where CodNot = :CodNot");
		/* CodNot es un  valor unívoco por lo tanto al hacer click en una notificación específica nos aseguramos que se guarde en el campo visto=1 que implica que ya se ha visto, ya que por defecto es 0, estas notificaciones persistirán hasta que se vean */
		$consulta->execute(array(
			':CodNot' => $CodNot
			));
	}
...
class amigos
{
	...
	public static function verificar($usua_enviador, $usua_receptor)
	{
		...
		/*Queremos comprobar si un usuario es amigo de otro usuario asi que cada uno puede entrar en categoría de usuario enviador o receptor indistintamente */
		$con = conexion();
	...
	/* $_SESSION['CodUsua'] siempre tendrá el código del usuario que esta navegando en el computador nas $_GET['CodUsua'] es una variable que se recibirá cada vez que se llame a perfil.php para procesados por usurio individual con respecto a $_SESSION['CodUsua']*/
	}
	...
	public static function codigos_amigos($CodUsua)
	{
        ...
        /*$amigos es el array que contiene los resultados de la función codigos_amigos en clase amigos, lo que sucede es que select group_concat(usua_enviador,',', usua_receptor) as amigos , como eso es el SELECT de la consulta efectuada , group_concat retorna un string con los valores No nulos de un grupo pasado como argumento en este caso retorna esa concatenación y son valores numéricos pero son pares de valores separados por ',' , por ello la necesidad de IN que se efectúa en post::mostrarTodo($amigos[0]['amigos']);, en el cual hay evidencia de que en IN se separan por comas los argumentos, tal cual como la variable $amigos: https://www.w3schools.com/sql/sql_in.asp.
        usando group_concat() : https://www.w3resource.com/mysql/aggregate-functions-and-grouping/aggregate-functions-and-grouping-group_concat.php
        */
	...
	public static function solicitudes($CodUsua)
	{
		...
		A.status != 1 es para las solicitudes que aún no han sido acceptadas que se le han enviado al usuario $CodUsua pasado como parámetro*/
		$con = conexion();
	...
	{
		/* Para aceptar una solicitud de amistad, y con set status = 1 actualizamos el estado en la base de datos para la siguiente vez se reconozca a un usuario que ya sería amigo */
		$con = conexion();
	...
	{
		/* Para eliminar una solicitud de amistad , para eliminar ese registro  de una solicitud de $CodAm pasado como parámetro */
		$con = conexion();
	...
	{
		/* Para contabilizar la cantidad de amigos, esta función es llamada desde perfil.php para mostrar ese valor contabilizado */
		$con = conexion();
