...
<?php
	...
	*/
	require('header.php');
	/*
		Require código html de cabecera, pero este código no finaliza el documento html, así que continua inmediatamente después con subir.php
	*/
	require('subir.php');
	...
	/*
		...
		1. header.php (a partir de ahí como nos encontramos en la cabecera, podemos o bien dirigirnos a  ver perfil con lo cual pefil.php completaría el </body></html> y o bien si queremos confirmar una solicitud.php de contacto nos seguimos quedando en el header por lo que ese archivo solicitud.php se ejecuta y le devuelve el control a header.php, también podemos ir a post.php o a cerrar.php , desde perfil.php(que puede cerrar el body y el documento html) la antepenúltima línea  del código se require publicacion.php con el objetivo de cargar todas las publicaciones del usuario, pero si nos seguimos quedando en el header y no hemos tomado la decisión de ver nuestro perfil aún, lo que podemos hacer también)
	*/
	...
	{
		/*
			Este condicional siempre se ejecuta independientemente si se suben o no archivos, así que !empty($_FILES) nunca esta vacia o esta incorrectamente procesada prueba de ello es el directorio "/subidos" presente en todas las publicaciones en la base de datos
		*/
		if(!empty($_FILES)){
			$destino = 'subidos/';
			...
			if($tipo1 == 'jpg' || $tipo1 == 'JPG' || $tipo1 == 'png' || $tipo1 == 'PNG')
			{
				...
				*/
				...
				if($archivoSinConf!==""){
					/*
						si $_FILES['archivo']['name']; esta vacío, por ejm al concatenar con "destino", ese $_FILES['archivo']['name']; nos retorna "", una cadena vacía, por ello se lo comparo con "" más no con NULL
					*/
		...
		*/
		...
		/*
			TODO: TODO: header('location: index.php') TODO:es para que no se vuelva a subir la publicación y carguemos nuevamente index.php, recordar que lo primero que lee este archivo index.php son  las funciones, las clases y los los "require" tanto el header.php y subir.php, como no se establecerá este condicional de "subir contenidos ,archivos", simplemente se cargarán después de lo anterior dicho, las publicaciones con require('publicacion.php');
			
			TODO: (IMPORTANTE ) TODO: Si ya publicamos con subir.php, que no tiene una finalización de código html, nos movemos a este condicional, y como ya hemos subido contenido nos movemos(obligadamente a más de subir un archivo(foto) debemos subir contenido o una potencial descripcion), retornamos y recargamos index.php con header('location: index.php');para actualizar la nueva publicación , luego esas variables $_POST['publicar'], $_FILES y $_POST['contenido' estarín vacías y al recargar con header('location: index.php'); por lo que tanto en un inicio como despueé de una publicación estas variables quedan vacías y solo se guardan en la base de datos para invocarlas cuando sean necesarias y luego  seguir a las siguientes líneas de código de index.php que finaliza el código con require('publicacion.php'); seguido de </body></html>
		*/
	...
	/*
		...
		Este condicional nos garantiza que si no estamos buscando , estamos mostrando todas las publicaciones de los amigos y de si mismo del usuario de sesión

		La función mostrarTodo($amigos[0]['amigos']); nos permitíra mostrar todas las publicaciones, [0] es la tabla 0, porque solo tenemos 1 tabla en esta consulta
		recordar que select group_concat(usua_enviador,',', usua_receptor) as amigos exactamente COMO AMIGOS, por ello se carga ese indice en ($amigos[0]['amigos']), por	que ese ['amigos'] contiene todos los group_concat(usua_enviador,',', usua_receptor) así que una cosa es $amigos al que se le asigna con la consulta $amigos=amigos::codigos_amigos($_SESSION['CodUsua']); y otra cosa es ['amigos']  que conforma $amigos[0]['amigos']), el 'amigos' el que hace referencia a group_concat(usua_enviador,',', usua_receptor) as amigos el que se retorna en amigos::codigos_amigos($_SESSION['CodUsua']);
	*/
